/*
 * Excellence Colorswatch
 * Developed By : Excellence technologies pvt. Lmt.
 * 
...
function setCorrectLabels() {
    ...
    jQuery('.product-options .input-box select').each(function() {
        ...
        if (jQuery(this).val() != '' && jQuery(this).val() != data.chooseText) {
            var optionPrice = data.attributes[attrId].options[(jQuery(this)[0].selectedIndex - 1)].price;
        ...
        if (!jQuery(this).is(":visible")) {
            ...
            options.forEach(function(cObj) {
                if (jQuery('option[value=' + cObj.id + ']').length > 0) {
                    ...
                    jQuery('#' + attrId).append('<div><img class="invalidOptions" id="' + cObj.id + '"src="' + getBaseDir() + 'colorswatch/' + attrId + '/' + cObj.id + '/img" alt="' + cObj.label + '"/></div>');
                }
            });
    ...
    jQuery(jQuery('.regular-price').children('span').text(currency[0] + lastPrice));
}
